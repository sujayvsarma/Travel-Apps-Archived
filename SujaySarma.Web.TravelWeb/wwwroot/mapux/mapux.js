...
class MapUX {
     ...
     */
    constructor(mapControlId, hostName, apiToken, subscriptionKey, startLat, startLon) {
        if (!document.getElementById(mapControlId)) {
            console.error('Error! Could not find a DOM element with Id #' + mapControlId);
            return;
        }

        this._setInitVars(hostName, apiToken, subscriptionKey, startLat, startLon);
    ...
    _initializeMaps(mapControlId, language, zoomLat, zoomLon) {
        ...
        var mapAuthOptions = {
            ...
            subscriptionKey: this._INIT_VARS.atlas.subscriptionKey
        };
        ...
        this._map.events.add('load', () => {
            ...
            canvas.title = 'Plan your travel itinerary';
            canvas.alt = 'Plan your travel itinerary';
        ...
        this._map.events.add('click', (e) => {
            this._getLocationInformation(new MapUXCoordinate(e.position[1], e.position[0]));
        });
    }
     ...
     * @param {MapUXCoordinate} latLon
     */
    _getLocationInformation(latLon) {
        this._showActivityAnimation('Searching for information...');

        var instance = this;
        var hostName = this._INIT_VARS.internal.hostName, token = this._INIT_VARS.internal.token;

        var ajaxSuccessCallback = function (data) {
            instance._getLocationInformationProcessResponse(data, instance);
        ...
        $.ajax(
            {
                method: 'GET',
                url: 'https://' + hostName + '/getinfo/' + token + '?lat=' + latLon.Latitude + '&lon=' + latLon.Longitude,
                cache: true,
                dataType: 'json',
     ...
     */
    _getLocationInformationProcessResponse(data, mapUxInstance) {
        ...
        $('#locationInfoDialog #locationInfo_country_flag').html('<img class="mr-2" style="width:16px;" src="https://countryflagsapi.com/png/' + data.country.alpha2Code + '" />');
        $('#locationInfoDialog #locationInfo_country_name').html(data.country.name);
        $('#locationInfoDialog #locationInfo_country_iso').html(data.country.alpha2Code);
        ...
        for (var i = 0; i < data.airports.length; i++) {
            ...
            airportInfo += '<td>' + (data.airports[i].municipality ? data.airports[i].municipality : data.airports[i].name) + '<br/>(' + data.airports[i].country + ')</td>';
            airportInfo += '<td>';
            ...
            if (!isLastLegAirport) {
                ...
                flyHereButton.html('<img src="/images/aircraft-' + flyHereButtonImage + '.svg" width="32" height="16" />' + data.airports[i].iata);
    ...
    _event_destinationAddOrEditDialog_saveButton_click(eventArgs) {
        ...
        var departureDate = new MapUXDate($('#destinationAddOrEditDialog #destinationAddOrEditDialog_field_departure').val());
    ...
    _event_destinationAddOrEditDialog_deleteButton_click(eventArgs) {
        ...
        if (legId < prevMaxLegId) {
            ...
            for (var i = legId; i < instance._itinerary.length; i++) {
                var legDep = new MapUXDate(instance._itinerary[i].Departure);
            ...
            for (var i = 0; i < this._itinerary.length; i++) {
                ...
                if ((leg.Origin !== null) && (drawnAirports.indexOf(leg.Origin.Iata) === -1)) {
                    this._addItineraryHaltMarker(
                        new MapUXCoordinate(leg.Origin.Lat, leg.Origin.Lon),
                        leg.Origin.Iata,
                        leg.Origin.Country
                    );
                ...
                if ((leg.Destination !== null) && (drawnAirports.indexOf(leg.Destination.Iata) === -1)) {
                    this._addItineraryHaltMarker(
                        new MapUXCoordinate(leg.Destination.Lat, leg.Destination.Lon),
     ...
     * @param {MapUXCoordinate} location
     * @param {string} iata
    ...
    _addItineraryHaltMarker(location, iata, country) {
        ...
        this._map.events.add('click', marker, (e) => {
            ...
            this._getLocationInformation(
                new MapUXCoordinate(e.target.options.position[1], e.target.options.position[0])
            );
        });
    }
     ...
     * @param {MapUXDate} departure - Date of departure from origin
     ...
     * @param {string} internalHostName - The hostname for the API services
     * @param {string} apiToken - Our API auth token
     * @param {string} subscriptionKey - Azure Maps Subscription Key
     * @param {number} lat - Latitude
     * @param {number} lon - Longitude
     */
    _setInitVars(internalHostName, apiToken, subscriptionKey, lat, lon) {
        this._initvars = {
            internal: {
